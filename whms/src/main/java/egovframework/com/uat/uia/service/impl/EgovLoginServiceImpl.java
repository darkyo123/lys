...
import egovframework.whms.otp.GoogleOTP;
...
public class EgovLoginServiceImpl extends EgovAbstractServiceImpl implements EgovLoginService {
	...
	public LoginVO actionLoginByEsntlId(LoginVO vo) throws Exception {
    	...
    	// 3. 결과를 리턴한다.
    	if (loginVO != null && !loginVO.getId().equals("") && !loginVO.getUsrPwd().equals("")) {
    		return loginVO;
    	} else {
    		loginVO = new LoginVO();
    	}
	...
	public LoginVO actionLogin(LoginVO vo) throws Exception {
    	...
    	// 1. 입력한 비밀번호를 암호화한다.
		String enpassword = EgovFileScrty.encryptPassword(vo.getUsrPwd(), vo.getId());
    	vo.setUsrPwd(enpassword);
    	...
    	if (loginVO != null && !loginVO.getId().equals("") && !loginVO.getUsrPwd().equals("")) {
    		...
    		String lstPwdDt = loginVO.getLstPwdDt();

    		// 패스워드 변경일자 이후 90일 체크
    		if(lstPwdDt == null || "".contentEquals(lstPwdDt)) {
    			loginVO.setPwdChangeYn("Y");
    		} else {
    			if(EgovDateUtil.checkWriteableDate(lstPwdDt, 90)) {
    				loginVO.setPwdChangeYn("Y");
    			} else {
    				loginVO.setPwdChangeYn("N");
    			}
    		}
	...
	public boolean searchPassword(LoginVO vo) throws Exception {
    	...
    	if (loginVO == null || loginVO.getUsrPwd() == null || "".equals(loginVO.getUsrPwd())) {
    		return false;
    	}
    ...
    public String processLoginIncorrect(LoginVO vo, Map<?,?> mapLockUserInfo) throws Exception{    	
    	...
    	String enpassword = EgovFileScrty.encryptPassword(vo.getUsrPwd(), EgovStringUtil.isNullToString(vo.getId()));
	...
	@Override
	public EgovMap getOtpKey(LoginVO loginVO) throws Exception {
		EgovMap result = new EgovMap();
		String checkVal = loginDAO.checkOtp(loginVO);
		if(checkVal == null || "".contentEquals(checkVal)) {
			EgovMap map = GoogleOTP.generate();
			result.put("otpkey", map.get("encodedKey"));
			map.put("id", loginVO.getId());
			loginDAO.updateOtp(map);
		} else {
			result.put("otpkey", checkVal);
		}
		return result;
	}

	@Override
	public boolean checkOtpKey(String otpKey, String otpVal) throws Exception {
		return GoogleOTP.checkCode(otpVal, otpKey);
	}

	@Override
	public void updateLoginFail(LoginVO loginVO) throws Exception {
		LoginVO lockVO = loginDAO.selectLockInfo(loginVO);
		if(lockVO != null) {
			String lockAt = lockVO.getLockAt();
			String lockCnt = lockVO.getLockCnt();
			if(!"Y".equals(lockAt)) {
				if(lockCnt != null && !"".contentEquals(lockCnt)) {
					int cnt = Integer.parseInt(lockCnt);
					cnt = cnt + 1;
					if(cnt >= 3) {
						loginVO.setLockCnt("3");
						loginVO.setLockAt("Y");
						loginDAO.insertLockHist(loginVO);
					} else {
						loginVO.setLockCnt(Integer.toString(cnt));
						loginVO.setLockAt("N");
					}
					loginDAO.updateLoginFail(loginVO);
				} else {
					loginVO.setLockCnt("1");
					loginVO.setLockAt("N");
					loginDAO.updateLoginFail(loginVO);
				}
			}
