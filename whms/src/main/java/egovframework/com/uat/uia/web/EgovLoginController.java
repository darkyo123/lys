...
public class EgovLoginController {
	...
	public String loginUsrView(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		if (isAuthenticated) {
			return "redirect:/uat/uia/actionMain.do?parentYn=N";
		}
	...
	public String actionLogin(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		try {
			...
			if(devPwd != null) {
				...
				loginVO.setUsrPwd(new String(devPwd));
			}
		} catch (Exception e) {
			e.printStackTrace();
			loginVO.setUsrPwd("");
		...
		if (resultVO != null && resultVO.getId() != null && !resultVO.getId().equals("")) {
			...
			String lockAt = resultVO.getLockAt();

			if(lockAt != null && "Y".contentEquals(lockAt)) {
				model.addAttribute("message", "로그인을 3회 실패하여 계정이 잠금상태입니다. 관리자에게 연락바랍니다");
				return "egovframework/com/uat/uia/EgovLoginUsr";
			} else {
...
//				request.getSession().setAttribute("TAATLoginId", resultVO.getId());
//
//	            EgovHttpSessionBindingListener listener = new EgovHttpSessionBindingListener();
//	            request.getSession().setAttribute(resultVO.getId(), listener);

				// 로그인 성공 로그 처리
				doInsertLoginLog("Y", resultVO, "I");
	...
	public String actionMain(ModelMap model, HttpServletRequest request) throws Exception {
		...
		*/
		...
		String main_page = "/whms/errorPage.do";

		if(list_menulist != null && list_menulist.size() > 0) {
			EgovMap map = (EgovMap) list_menulist.get(0);
			main_page = "/sym/mnu/mpm/EgovMainMenuIndex.do?menuNo="+map.get("menuNo")+"&chkURL=dir";
		} else {
			request.setAttribute("errorMsg", "생성된 메뉴가 없습니다");
		}
...
//		RequestContextHolder.getRequestAttributes().removeAttribute("TAATLoginId", RequestAttributes.SCOPE_SESSION);
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/uat/uia/getOtpKey.do")
	public ModelAndView getOtpKey(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if (resultVO != null && resultVO.getId() != null && !resultVO.getId().equals("")) {
			EgovMap result = loginService.getOtpKey(loginVO);
			modelAndView.addObject("result", result);
			modelAndView.addObject("pwdChangeYn", resultVO.getPwdChangeYn());
			modelAndView.addObject("lockYn", resultVO.getLockAt());
		} else {
			...
			loginService.updateLoginFail(loginVO);
			modelAndView.addObject("result", null);
			modelAndView.addObject("message", egovMessageSource.getMessage("fail.common.login",request.getLocale()));
			modelAndView.addObject("lockYn", null);
		}
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/uat/uia/checkOtpKey.do")
	public ModelAndView checkOtpKey(HttpServletRequest request, ModelMap model) throws Exception {

		ModelAndView modelAndView = new ModelAndView();
    	modelAndView.setViewName("jsonView");

    	String otpKey = EgovStringUtil.isNullToString(request.getParameter("otpKey"));
    	String otpVal = EgovStringUtil.isNullToString(request.getParameter("otpVal"));

    	if(loginService.checkOtpKey(otpKey, otpVal)) {
	...
	private void doInsertLoginLog(String successYn, LoginVO loginVO, String loginType) {
		LoginLog loginLog = new LoginLog();
    	loginLog.setLoginId(loginVO.getId());
        loginLog.setLoginIp(loginVO.getIp());
        loginLog.setLoginMthd(loginType); // 로그인:I, 로그아웃:O
        loginLog.setErrOccrrAt(successYn.equals("Y") ? "N" : "Y");
        loginLog.setErrorCode("");
        loginLog.setLoginYn(successYn);
        try {
	...
	/** OTP 변경 팝업 */
	@RequestMapping(value="/uat/uia/otpPopup.do")
	public String otpPopup( @ModelAttribute("loginVO") LoginVO loginVO
			, HttpServletRequest request
			, HttpServletResponse response
			, ModelMap model) throws Exception {
		return "egovframework/com/uat/uia/OtpPopup";
	}

	/** 패스워드 변경 팝업 */
	@RequestMapping(value="/uat/uia/pwdChangePopup.do")
	public String pwdChangePopup( @ModelAttribute("loginVO") LoginVO loginVO
			, HttpServletRequest request
			, HttpServletResponse response
			, ModelMap model) throws Exception {
		return "egovframework/com/uat/uia/PwdChangePopup";
	}
